#!/usr/bin/env python3
"""Sof Python PDF matn ekstraktori (oddiy TrueType fontlar uchun).

"7.Garri Potter va ajal tuhfalari.pdf" uchun moslangan:
- Simple TrueType subset fontlar (DejaVuSans), 1-baytli kodlar
- ToUnicode CMap (kirill yozuvi)
- Matn TJ/Tj massivlarida hex <..> yoki literal (..) ko'rinishida
- Resources / Font havolalar (indirect ref) orqali berilgan
"""
import re, zlib, sys
...
def parse_simple_widths(fontbody):
    """/FirstChar + /Widths[...] -> {code: width(1/1000em)}."""
    widths = {}
    ...
    for i, n in enumerate(nums):
        try:
            widths[first + i] = float(n)
        except ValueError:
            pass
    return widths

def resolve_ref(objs, body, key):
    """body ichidagi /key N 0 R havolasini obyekt raqamiga qaytaradi."""
...
def font_to_data(fontnum, objs, cache):
    ...
    cache[fontnum] = (cm, widths, 500.0)
    cache[fontnum + 0] = cache[fontnum]
    return cache[fontnum]

def page_fonts(page_body, objs, cache):
    """Sahifa /Resources -> /Font dict -> {fontname: (cmap, widths, dw)}.
    Resources va Font indirect ref bo'lishi mumkin."""
    fonts = {}
    # Resources dict
    res = None
...
def decode_hex_codes(h):
    ...
    for i in range(0, len(h) - 1, 2):
        codes.append(int(h[i:i+2], 16))
    return codes

def decode_lit(s):
    """PDF literal string -> 1-baytli kodlar ro'yxati."""
    out = []
    i = 0
...
def interpret(stream, fonts):
    ...
    for m in tok_re.finditer(stream):
        if m.group('num') is not None:
            ...
            operands.append(('lit', m.group('lit').decode('latin1')))
            ...
            if op == 'q':
                ...
                for o in reversed(operands):
                    ...
                    if isinstance(o, tuple) and o[0] == 'lit':
                        emit(decode_lit(o[1])); break
            elif op == 'TJ':
                for o in reversed(operands):
                    if isinstance(o, tuple) and o[0] == 'arr':
                        # arr ichidagi <hex>, (lit), va kerning raqamlari tartibda
                        for tm2 in re.finditer(r'<([0-9A-Fa-f\s]*)>|\(((?:[^()\\]|\\.)*)\)|([-+]?\d*\.?\d+)', o[1]):
                            if tm2.group(1) is not None:
                                emit(decode_hex_codes(tm2.group(1)))
                            elif tm2.group(2) is not None:
                                emit(decode_lit(tm2.group(2)))
                            elif tm2.group(3) is not None:
                                kern = float(tm2.group(3))
                                # TJ kerning: text fazosida -kern/1000*fontsize siljish
                                shift = -kern / 1000.0 * fontsize
